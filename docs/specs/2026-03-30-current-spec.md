...
- [ ] REQ-22a: `pnpm dev` 必须启动前端 dev server，监听 `http://localhost:1420` 供 Tauri 开发模式连接。
- [ ] REQ-22b: `pnpm dev:app` 必须启动 Dora 开发版，并复用前端 dev server。
- [ ] REQ-22c: 用户必须可以通过 `pnpm build:app` 构建 Dora.app。
- [ ] REQ-22d: `pnpm build` 必须只执行前端构建，不得递归触发 Tauri 打包流程。
...
- [ ] REQ-33c: 陪伴模式下拉框当前阶段使用 antd Select 实现，并保持与现有设置区块样式协调。
- [ ] REQ-33d: 聊天、便签、记忆、设置四个主面板的头部高度与关闭按钮视觉必须保持一致。
- [ ] REQ-33e: 记忆列表卡片应预留置顶/排序视觉位，但当前阶段不实现真实排序交互。
- [ ] REQ-33f: 首页主卡需要降低纯白板感，保留更柔和的层次与说明文案。
- [ ] REQ-33g: 首页操作按钮需要维持明确主次关系，且记忆卡片与设置页底部操作区的视觉密度应继续收紧。
- [ ] REQ-33h: 首页操作按钮可增加轻量图标提示，记忆卡片 hover 层级与设置页表单控件高度需继续统一。
...
- [ ] AC-31: Given 用户进入设置页, when 打开陪伴模式选择器, then 使用 antd Select 交互并能正确保存选中的模式。
- [ ] AC-32: Given 用户在聊天、便签、记忆、设置面板之间切换, when 查看头部区域, then header 高度与关闭按钮风格保持一致。
- [ ] AC-33: Given 用户打开记忆面板且存在记忆条目, when 查看单条记忆卡片, then 可看到置顶状态与排序位的视觉预留。
- [ ] AC-34: Given 用户回到首页, when 查看主卡区域, then 卡片层次更柔和且包含 Dora 的简短说明文案。
- [ ] AC-35: Given 用户查看首页、记忆卡片和设置页底部操作区, when 对比主次层级, then 主按钮更突出且其他操作区更紧凑统一。
- [ ] AC-36: Given 用户查看首页操作按钮、记忆列表和设置页表单, when 观察交互反馈, then 可看到轻量图标、卡片 hover 层级和统一的控件高度。
...
| 2026-04-01 | 统一聊天/便签/记忆/设置面板 header 与关闭按钮视觉，记忆列表增加置顶/排序视觉预留，并弱化首页白板感 | 用户继续要求 UI 优化 |
